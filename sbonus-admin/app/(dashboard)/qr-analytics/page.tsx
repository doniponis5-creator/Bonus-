...
const tooltipStyle = {
  ...
  color: '#e2eaf6', fontSize: 13, boxShadow: '0 8px 32px rgba(0,0,0,0.5)', padding: '10px 14px',
};

const DEVICE_COLORS: Record<string, string> = { mobile: '#22c55e', desktop: '#3b82f6', tablet: '#f97316', unknown: '#8899aa' };
...
export default function QRAnalyticsPage() {
  ...
  const deviceData = Object.entries(data.device_breakdown || {}).filter(([,v]) => (v as number) > 0).map(([k, v]) => ({
    ...
    value: v as number, color: DEVICE_COLORS[k] || '#8899aa',
  }));
    ...
    <div>
      ...
      <div className="grid-4" style={{ marginBottom: 24 }}>
        {[
          ...
          { label: 'Сегодня', value: data.scans_today, color: '#22c55e' },
          { label: 'За неделю', value: data.scans_this_week, color: '#3b82f6' },
      ...
      <div className="grid-2" style={{ marginBottom: 24 }}>
        <div className="card">
          ...
          {data.top_sources?.length > 0 ? data.top_sources.map((s: any, i: number) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #1c2a3a' }}>
              <span style={{ fontSize: 13, fontWeight: 600 }}>{s.source}</span>
              <span style={{ fontSize: 14, fontWeight: 800, color: 'var(--accent)' }}>{s.count}</span>
            </div>
          )) : <p style={{ color: 'var(--text3)', fontSize: 13 }}>Нет данных</p>}
        </div>
        <div className="card">
          ...
          {data.top_campaigns?.length > 0 ? data.top_campaigns.map((c: any, i: number) => (
            <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid #1c2a3a' }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 800, color: '#3b82f6' }}>{c.count}</span>
            </div>
          )) : <p style={{ color: 'var(--text3)', fontSize: 13 }}>Нет данных</p>}
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            ...
            <tbody>
              {scans.length === 0 && (
                <tr><td colSpan={5} style={{ padding: 32, textAlign: 'center', color: '#8899aa' }}>Нет сканирований</td></tr>
              )}
              {scans.map((s, i) => (
                <tr key={i}>
                  <td style={{ padding: '12px 14px', borderBottom: '1px solid #1c2a3a', fontSize: 12, fontFamily: 'monospace', color: 'var(--accent)' }}>{s.qr_code?.slice(0, 12)}...</td>
                  <td style={{ padding: '12px 14px', borderBottom: '1px solid #1c2a3a', fontSize: 13, fontWeight: 600 }}>{s.customer_name || '—'}</td>
                  <td style={{ padding: '12px 14px', borderBottom: '1px solid #1c2a3a', fontSize: 12, color: '#8899aa' }}>{s.utm_source || 'direct'}</td>
                  <td style={{ padding: '12px 14px', borderBottom: '1px solid #1c2a3a', fontSize: 12, color: '#8899aa' }}>{s.device_type || '—'}</td>
                  <td style={{ padding: '12px 14px', borderBottom: '1px solid #1c2a3a', fontSize: 12, color: '#8899aa' }}>

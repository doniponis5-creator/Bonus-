...
const TIER_META: Record<string, { Icon: LucideIcon; color: string }> = {
  ...
  Gold:     { Icon: Trophy, color: '#FFE600' },
  Platinum: { Icon: Gem,    color: '#8b5cf6' },
};

const DEFAULT_META = { Icon: Star, color: '#3b82f6' };
...
export default function TiersPage() {
  ...
  return (
    <div>
      <h1 style={{display: 'flex', alignItems: 'center', gap: 8,  fontSize: 24, fontWeight: 700, marginBottom: 24, flexWrap: 'wrap' as any }}><Trophy size={24} /> Уровни бонусной программы</h1>

      {/* Карточки уровней из БД */}
      {loading ? (
        <div style={{ color: 'var(--text2)', marginBottom: 24 }}><Loader2 className="animate-spin" style={{marginRight: 8, display: 'inline'}} size={16} /> Загрузка...</div>
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 16, marginBottom: 32 }}>
          {tiers.map(t => {
            ...
            return (
              <div className="card" key={t.id} style={{ textAlign: 'center', borderColor: `${meta.color}30` }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 700, color: meta.color }}>{t.name}</div>
                <div style={{ fontSize: 36, fontWeight: 700, color: meta.color, margin: '8px 0' }}>
                  {t.bonus_percent}%
                </div>
          ...
          {tiers.length === 0 && (
            <div style={{ color: 'var(--text2)', gridColumn: '1/-1' }}>Уровни не найдены</div>
          )}
        </div>
      )}
      ...
      <div className="card" style={{ maxWidth: 520 }}>
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              <label style={{ display: 'block', fontSize: 12, color: 'var(--text2)', marginBottom: 6 }}>Мин. сумма (сом) *</label>
              <input className="input" name="min" type="number" min="0" placeholder="0" required />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 12, color: 'var(--text2)', marginBottom: 6 }}>Бонус % *</label>
              <input className="input" name="pct" type="number" min="0.1" max="100" step="0.1" placeholder="3" required />
            </div>
          </div>
          <div>
            <label style={{ display: 'block', fontSize: 12, color: 'var(--text2)', marginBottom: 6 }}>Макс. списание % (по умолч. 30)</label>
